...
def create_upload_form_attributes(prefix, input_type, name):
    ...
    :type input_type: str
    :param prefix: field type (file, raw, url)
    :type name: str
    ...
    attributes['data-' + prefix + 'source-' + input_type] = _(name)
    return attributes
...
class TemplateForm(forms.SelfHandlingForm):
    ...
    def clean(self):
        ...
        self.clean_uploaded_files('template', cleaned, files)
        self.clean_uploaded_files('environment', cleaned, files)
    ...
    def clean_uploaded_files(self, prefix, cleaned, files):
