...
class StackTests(test.TestCase):
    ...
    def test_index_paginated(self):
        ...
        url = "?".join([reverse('horizon:project:stacks:index'),
                    "=".join([tables.StacksTable._meta.pagination_param,
                              stacks[2].id])])
        res = self.client.get(url)
...
[u'<Traceback>']."""
    ...
    def test_clean_file_upload_form_url(self):
        ...
        t.clean_uploaded_files('template', precleaned, {})
    ...
    def test_clean_file_upload_form_valid_data(self):
        ...
        t.clean_uploaded_files('template', precleaned, files)
        self.assertEqual(
            json_str,
